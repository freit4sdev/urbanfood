...
from src.auth.client_signup import ClientSignupWindow


class ClientAuthWindow:
    
    def __init__(self, parent, on_close_callback=None):
        self.parent = parent
        self.on_close_callback = on_close_callback
        self.window = tk.Toplevel(parent)
        self.window.title("UrbanFood - Login Cliente")
    ...
    def _handle_login(self):
        ...
        try:
            cursor.execute(
                "SELECT id, name, email, user_type, is_active, is_blocked FROM users WHERE email = ? AND password = ? AND user_type = 'client'",
                (email, password)
            )
            ...
            self._close_window()
            from src.client.dashboard import ClientDashboard
            ClientDashboard(self.parent, self.user_data)
    ...
    def _open_signup(self):
        signup_window = ClientSignupWindow(self.window)
