...
class AdminAuthWindow:
    ...
    def __init__(self, parent, on_close_callback=None):
        ...
        self.window.title("UrbanFood - Login Administrador")
    ...
    def _handle_login(self):
        ...
        try:
            cursor.execute(
                "SELECT id, name, email, user_type, is_active, is_blocked FROM users WHERE email = ? AND password = ? AND user_type = 'admin'",
                (email, password)
            )
            ...
            if user['is_blocked']:
                messagebox.showerror("Erro", "Sua conta está bloqueada.")
                return
            ...
            AdminDashboard(self.parent, self.user_data)
